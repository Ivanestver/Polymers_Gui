...
class CalcAlg:
    def __init__(self, globuls_count = 1, polymers_count = 1, accept_threshold = 0.1):
        self.GLOBULS_COUNT = globuls_count
        self.POLYMERS_COUNT = polymers_count
        self.accept_threshold = accept_threshold
        self.finished_polimers_labels = ['C', 'N', 'H', 'O', 'F', 'Na', 'Mg', 'Al', 'P', 'S']

    def get_continuations(self, k_free, available_cells):
    ...
    def get_next_config(self, curr_config: Polymer, continuation):
        config_copy = curr_config.copy()
        config_copy.add_monomer(tuple(continuation))
        return config_copy


    def get_length_metrics(self, start_point, end_point):
        return math.sqrt(
            (start_point[1] - end_point[1]) ** 2 + (start_point[0] - end_point[0]) ** 2
        )

    def save_polymer_mol_into_file(self, path_to_save_dir, file_number, finished_polimers: list[Polymer]):
        ...
        for pol_number, pol in enumerate(finished_polimers):
            ...
            for i, monomer in enumerate(pol):
                label = self.finished_polimers_labels[pol_number]
                if i == 0:
                    label = 'Fe'
                if i == pol.len() - 1:
                    label = 'Cu'
        ...
        file_name = f'{path_to_save_dir}/polymer_{file_number}_{self.accept_threshold}.mol2'
    ...
    def calc(self, path_to_save_dir: str = None) -> list[Polymer]:
        ...
        is_stepping_back = False
        for epoch in range(self.GLOBULS_COUNT):
            print(f'epoch = {epoch}')
            finished_polimers = []
            field = Field()
            polymers = [Polymer(field) for i in range(self.POLYMERS_COUNT)]
            polymers_cache = [[] for i in range(self.POLYMERS_COUNT)]
            for p in polymers:
                new_pos = field.define_start_position()
                p.add_monomer(new_pos)
                field.make_filled(new_pos)

            while sum([max_monomers_count - polymer.len() for polymer in polymers]) != 0:
                for i in range(len(polymers)):
                    ...
                    continuations = self.get_continuations(len(available_cells), available_cells)
                    potential_configs = [self.get_next_config(polymer, next_step) for next_step in continuations]
                    U_current = polymer.calc_energy()
                    while True:
                        choice = np.random.randint(0, len(potential_configs))
                        next_config = potential_configs[choice]
            ...
            if path_to_save_dir is not None:
                self.save_polymer_mol_into_file(epoch, finished_polimers)
